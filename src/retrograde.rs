...
fn is_valid_predecessor(board: &Board, us: Color, them: Color) -> bool {
    if let Some(them_king) = board.king_of(them) {
        ...
        if (pawn_attacks & board.pawns() & board.by_color(us)).any() {
            return false;
        }

        let knight_attacks = shakmaty::attacks::knight_attacks(them_king);
        if (knight_attacks & board.knights() & board.by_color(us)).any() {
            return false;
        }

        let king_attacks = shakmaty::attacks::king_attacks(them_king);
        if (king_attacks & board.kings() & board.by_color(us)).any() {
            return false;
        }

        let bishop_attacks = shakmaty::attacks::bishop_attacks(them_king, occupied);
        if (bishop_attacks & (board.bishops() | board.queens()) & board.by_color(us)).any() {
            return false;
        }

        let rook_attacks = shakmaty::attacks::rook_attacks(them_king, occupied);
        if (rook_attacks & (board.rooks() | board.queens()) & board.by_color(us)).any() {
            return false;
        }
    }
    true
...
pub fn inverse_moves(position: &Chess, parents: &mut Vec<Chess>) {
            ...
            for off in capture_offsets {
                    ...
                    && src.distance(sq) == 1
                {
                    ...
                    if let Some(captured_sq) = sq.offset(direction)
                        && board.piece_at(src).is_none()
                        && board.piece_at(captured_sq).is_none()
                    {
    ...
    for (k_sq, r_sq, k_src, r_src, _) in castling_unmoves {
        if board.role_at(k_sq) == Some(Role::King)
            && board.color_at(k_sq) == Some(us)
            && board.role_at(r_sq) == Some(Role::Rook)
            && board.color_at(r_sq) == Some(us)
            && board.piece_at(k_src).is_none()
            && board.piece_at(r_src).is_none()
        {
            ...
            if castling_legal {
                ...
                for check_sq in [k_src, k_pass] {
                    ...
                    if (pawn_attacks & prev_board.pawns() & prev_board.by_color(them)).any() {
                        attacked = true;
                        break;
                    }

                    let knight_attacks = shakmaty::attacks::knight_attacks(check_sq);
                    if (knight_attacks & prev_board.knights() & prev_board.by_color(them)).any() {
                        attacked = true;
                        break;
                    }

                    let king_attacks = shakmaty::attacks::king_attacks(check_sq);
                    if (king_attacks & prev_board.kings() & prev_board.by_color(them)).any() {
                        attacked = true;
                        break;
                    }
                    ...
                    if (bishop_attacks
                        & (prev_board.bishops() | prev_board.queens())
                        & prev_board.by_color(them))
                    .any()
                    {
                        attacked = true;
                        break;
                    ...
                    if (rook_attacks
                        & (prev_board.rooks() | prev_board.queens())
                        & prev_board.by_color(them))
                    .any()
                    {
                        attacked = true;
