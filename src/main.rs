...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some("--generated-depth-two-value-unique-signature-left-supply-value-spread-selection") => {
            let (rows_per_family, candidate_pair_limit) =
                parse_generated_depth_two_value_unique_signature_left_supply_value_spread_selection_args(
                    args,
                );
            let report =
            ...
            println!(
                "{}",
                serde_json::to_string_pretty(&report).expect(
                    "generated depth-two value-unique signature left-supply value-spread selection report must serialize",
                )
            );
            return;
        }
        Some(
            ...
            let rows_per_family =
                parse_generated_depth_two_value_unique_signature_left_supply_dynamic_pairing_preflight_rows(args);
            let report =
                dataset_label::generated_depth_two_value_unique_signature_left_supply_dynamic_pairing_preflight_report(
                    rows_per_family,
                );
            println!(
                "{}",
                serde_json::to_string_pretty(&report).expect(
                    "generated depth-two value-unique signature left-supply dynamic pairing preflight report must serialize",
                )
            );
            return;
        }
        ...
        Some(
            "--generated-depth-two-value-unique-signature-all-left-all-right-dynamic-pairing-preflight",
        ) => {
            let rows_per_family =
                parse_generated_depth_two_value_unique_signature_all_left_all_right_dynamic_pairing_preflight_rows(args);
            let report =
                dataset_label::generated_depth_two_value_unique_signature_all_left_all_right_dynamic_pairing_preflight_report(
                    rows_per_family,
                );
            println!(
                "{}",
                serde_json::to_string_pretty(&report).expect(
                    "generated depth-two value-unique signature all-left all-right dynamic pairing preflight report must serialize",
                )
            );
            return;
        }
...
Commands:\n\
  ...
  --generated-depth-two-value-unique-signature-all-left-all-right-dynamic-pairing-preflight [--rows-per-family N]\n\
...
fn parse_generated_depth_two_value_unique_signature_left_supply_value_spread_selection_args(
    mut args: impl Iterator<Item = String>,
) -> (usize, usize) {
    let command = "--generated-depth-two-value-unique-signature-left-supply-value-spread-selection";
    let mut rows_per_family = dataset_label::DEFAULT_SIGNATURE_TARGET_DIAGNOSTIC_ROWS_PER_FAMILY;
    let mut candidate_pair_limit = 2_500usize;
    while let Some(arg) = args.next() {
        match arg.as_str() {
...
fn parse_generated_depth_two_value_unique_signature_left_supply_dynamic_pairing_preflight_rows(
    mut args: impl Iterator<Item = String>,
) -> usize {
    parse_rows_per_family_arg(
        &mut args,
...
fn parse_generated_depth_two_value_unique_signature_all_left_all_right_dynamic_pairing_preflight_rows(
    mut args: impl Iterator<Item = String>,
) -> usize {
    parse_rows_per_family_arg(
        &mut args,
